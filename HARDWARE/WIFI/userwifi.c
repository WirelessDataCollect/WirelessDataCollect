...
u8 wifi_send_package()
{
	...
	if(Time_Sync_Flag==1)
	{
		...
		rsi_send_ludp_data(socketDescriptor_txrx, &adc_queue.arr[Head],Length, RSI_PROTOCOL_UDP_V4, (uint8 *)destIp_txrx, moduleSocket_txrx, &bytes_sent);
	...
	if(queue_length(adc_queue) >= UDP_SEND_SIZE - 10 )//我觉得>=比较好，万一没来得及发现==UDP_SEND_SIZE - 10的情况呢
	{
		...
		rsi_send_ludp_data(socketDescriptor_txrx, &adc_queue.arr[Head],UDP_SEND_SIZE, RSI_PROTOCOL_UDP_V4, (uint8 *)destIp_txrx, moduleSocket_txrx ,&bytes_sent);
	}
	return 1;
}
...
void Send_Sync_Time(void)
{
	...
	rsi_send_ludp_data(socketDescriptor_sync,time ,SYNC_TIME_BYTES, RSI_PROTOCOL_UDP_V4, (uint8 *)destIp_sync, moduleSocket_sync ,&bytes_sent);
}
#endif

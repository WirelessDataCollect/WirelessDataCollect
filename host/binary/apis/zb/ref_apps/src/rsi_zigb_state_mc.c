 ...
 *
 *  @brief API: ZigBee state machine function
 *
 *  @section Description
...
uint8_t *rcv_pkt_buffer=NULL;
int rsi_zigb_state_machine(uint8_t *pkt_buffer)
{
...
#ifdef ZB_API_TEST
  static uint8_t        ret_val=0,nwk_state_once=0;  
#endif
  rsi_zigb_uCmdRsp      *resp;
  rcv_pkt_buffer = pkt_buffer;
    ...
    //if (app_cb_ptr->pkt_pending == RSI_TRUE)
    {
...
#endif      
      resp = rsi_zigb_app_frame_process(rcv_pkt_buffer);
      if (!resp) {
#ifdef ENABLE_DEBUG_PRINTS    
      ...
      switch (app_cb_ptr->fsm_state)
      {
        ...
        case FSM_INIT_ENDDEV_ROUTER:
        {
          if (cmd_type == ZIGBEESETSIMPLEDESCRIPTOR || cmd_type == ZIGBEESTACKISUP)
          {
            ...
            if( rsi_status == RSI_ZB_FAIL) {
              RSI_DPRINT(RSI_PL1,"Unable to Set Simple Descriptor \n");
              return RSI_ZB_FAIL;
            }
        ...
        case FSM_SCAN_DONE:
        {
          ...
          rsi_zigb_join_network(device_type, rsi_zigb_app_info.nwkinfo.channel, 0x0f, rsi_zigb_app_info.nwkinfo.extendedPanId);
#ifdef ENABLE_DEBUG_PRINTS    
        ...
        case FSM_JOIN_NETWORK:
        {
          ...
          if (app_cb_ptr->fsm_state == FSM_ZB_CONNECTED)
          {
            ...
            else
            {
              ...
              app_cb_ptr->fsm_state = FSM_ZB_MATCH_DESC_RESP;
              if(rsi_status != RSI_ZB_SUCCESS) 
              {
#ifdef ENABLE_DEBUG_PRINTS    
                RSI_DPRINT(RSI_PL1,"Error in handling scan request \n");
        ...
        case FSM_ZB_MATCH_DESC_RESP:
        {
          //rsi_status = rsi_zigb_handle_data(cmd_type,intf_type);
          switch(cmd_type){
            case ZDPSENDMATCHDESCRIPTORSREQUEST:
              {
                ...
                {
#ifdef ENABLE_DEBUG_PRINTS        
                  RSI_DPRINT(RSI_PL1,"Match desc response fail \n");
#endif        
                  return RSI_ZB_FAIL;
                }
#ifdef ENABLE_DEBUG_PRINTS        
            ...
            case ZIGBEEENDDEVICEPOLLFORDATA:
              {
                ...
                {
#ifdef ENABLE_DEBUG_PRINTS        
                RSI_DPRINT(RSI_PL1,"Polling for Match desc response \n");
#endif        
                 rsi_zigb_end_device_poll_for_data();
                }
               }
        ...
        case FSM_API_TEST:
        {
          ...
          if(!nwk_state_once)
          {
            ...
            rsi_zigb_api_test();
#else
            ret_val = rsi_zigb_api_test(resp);
            if(!ret_val)
            {
#ifdef ENABLE_DEBUG_PRINTS    
            RSI_DPRINT(RSI_PL1,"TestCasesExecuted: %d \nTestCasesFailed: %d\nTestCasesSucceded %d\n ",TestCasesExecuted, TestCasesFailed,TestCasesSucceded );
...
UNKNOWN_STATE: 
#ifdef ENABLE_DEBUG_PRINTS    
  RSI_DPRINT(RSI_PL1," cmd_id: %x : state: %d  \n",cmd_type,app_cb_ptr->fsm_state);
#endif  
  return RSI_ZB_FAIL;
}
    ...
    if (cmd == APPSCANCOMPLETEHANDLER)
    {
      ...
      else
      {
        ...
        RSI_DPRINT(RSI_PL1,"Network found handler\n");
          ...
          /* Check for a specific network and then Join.
           * Return/ Rescan if nwk not found */
          rsi_zigb_join_network(app_cb_ptr->device_type, rsi_zigb_app_info.nwkinfo.channel, 0x0f, rsi_zigb_app_info.nwkinfo.extendedPanId);
      }
      app_info->status_var.scanReqSent = 0;
...
/*===========================================================================
 *
 * @fn          void rsi_zigb_init_struct(void)
 * @brief       Initialize ZigBee structues and variables
 * @param[in]   none
 * @param[out]  none
 * @return      none

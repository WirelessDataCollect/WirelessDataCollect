...
#define rsi_getpriv(a)             netdev_priv(a)
...
/*Function prototypes*/
/* net device */
struct net_device* rsi_allocdev(INT32 sizeof_priv);
...
struct net_device* rsi_netdevice_op(VOID);

INT32  rsi_xmit(struct sk_buff *skb, struct net_device *dev);
INT32 rsi_transmit_thread (PVOID pContext);
INT32  rsi_indicate_packet(PRSI_ADAPTER Adapter, UINT8 *DataRcvPacket,UINT32 pktLen);

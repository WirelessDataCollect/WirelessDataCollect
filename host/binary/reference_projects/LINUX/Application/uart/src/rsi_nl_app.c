 ...
 *     rsi_alloc_and_init_cmdbuff_dev_to_driver
...
#include "rsi_nic.h"
#ifdef BT_ENABLE
#include "rsi_bt_app.h" //since rsi_enqueue_to_rcv_q calls protocol APP CB
#endif
 ...
 * Function Prototypes
 */
static int32 rsi_create_nl_socket(int32 protocol, int32 groups);
...
{
  //! Takes packet from NL socket. And queues the packet
...
/*=================================================*/
/**
 * @fn          uint8 *rsi_prepare_nl_cmdbuff(uint8 *payload,
 *                                       uint16 payload_size)
 * @brief       To allocate and initialise the command buffer.
 * @param[in]   pkt_struct_t *Pkt
 * @param[out]  none
 ...
 * @section description
 * This API is used to allocate a buffer for the command to send and
 * initializing it with all the header bytes and payload filled.
 */
uint8 *rsi_prepare_nl_cmdbuff(uint8 *payload, uint16 payload_size)
{
  uint8          *cmd_buff;
  rsi_nlPkt_t    *req;
  struct nlattr  *na;
  ...
  RSI_DPRINT(RSI_PL14,"\nrsi_alloc_and_init_cmdbuff\n");
#endif
  cmd_buff = rsi_malloc(payload_size + RSI_NL_HEAD_SIZE);
  req = (rsi_nlPkt_t *)cmd_buff;
  ...
  na = (struct nlattr *) GENLMSG_DATA(cmd_buff);
  na->nla_type = 1; //!DOC_EXMPL_A_MSG
  na->nla_len = payload_size + NLA_HDRLEN + 2; //!message length
  memcpy(NLA_DATA(na), payload, payload_size);

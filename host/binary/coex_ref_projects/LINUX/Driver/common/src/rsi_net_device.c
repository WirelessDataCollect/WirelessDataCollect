...
VOID rsi_deinit_interface(PVOID pContext)
{
  ...
  {
    free_irq(SPI_INTR_GPIO_PIN,Adapter->net_device);
  }
#endif
  return;
}
...
)
{
  ...
  dev = alloc_netdev(sizeof_priv,"rsi_wlan",ether_setup);
  return dev;
}
    ...
    case OID_WSC_GET_STATUS:
      {
        ...
        if(copy_to_user(wrq->u.data.pointer,&retval,sizeof(UINT8)))
        {
...
/*==============================================*/
/**
 * @fn          struct net_device* rsi_netdevice_op(VOID)
...
)
{
  ...
  dev = rsi_allocdev(sizeof(RSI_ADAPTER));
  ...
  {
    RSI_DEBUG(RSI_ZONE_ERROR,
              "rsi_netdevice_op: Registration of net-device failed\n");
    free_netdev(dev);
    return NULL;
  }
...
void rsi_usb_disconnect(struct usb_interface *intf)
{
 ...
 if(Adapter->rx_usb_urb[USB_WLAN]->transfer_buffer)
 {
  ...
  rsi_unregisterdev(Adapter->net_device);
...
#endif
{
  ...
  Adapter = rsi_getpriv(dev); /*we can also use dev->priv;*/

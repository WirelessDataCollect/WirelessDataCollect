 ...
 */
INT16 rsi_uart_send(uint8_t *ptrBuf, UINT16 bufLen)
{
  INT16 retval = 0, i;
#ifdef RSI_DEBUG_PRINT

  RSI_DPRINT(RSI_PL13,"TX Len of the packet: %d\n", (bufLen - 16));
  for (i=0; i<bufLen; i++) {
    if ((i % 16 == 0) && (i != 0)) {
      RSI_DPRINT(RSI_PL13, "\n");
    }
    RSI_DPRINT (RSI_PL13, "0x%02x ", ptrBuf[i]);
  }
  RSI_DPRINT(RSI_PL13, "\n");
 ...
 */
INT16 rsi_uart_recv(uint8_t *ptrBuf, UINT16 bufLen)
{
  INT16 i = 0;
  uint8_t ch = 0;
#ifdef WINDOWS
